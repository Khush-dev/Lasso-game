...
string getTip(){        //RETURNS A RANDOM TIP FOR THE USER
  ...
  "A Magnet lasts for 5 seconds |=_=|  ", "Beware of Bombs! |=_*| ", "Red coins are Magnetic '-' ", "Longer the portal, faster the coin" };
...
void Coin::resetCoin() {
  double coin_speed = COIN_SPEED + randuv(-30,30);
...

  {
    ...
    while(true){
      ...
      if(mouseButtonPressEvent(event)){
        ...
        y = event.xbutton.y;
        x = event.xbutton.x;
        if(abs(x-WINDOW_X/2)<textWidth("(1). Easy")/2 && abs(y-WINDOW_Y/2-20)<textHeight()/2 ){
          difficulty = 1;
          break;
        }
        else if(abs(x-WINDOW_X/2+80)<textWidth("(2). Medium")/2 && abs(y-WINDOW_Y/2-20)<textHeight()/2 ){
          difficulty = 2;
          break;
        }
