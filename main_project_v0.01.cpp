...
#define n 2 //number of coins
#define INIT_LIVES 5        //INITIAL NUMBER OF LIVES
bool i_coin_vis[2] = {true,false};
//bool coin_vis[2] = {true,false};
...
class Lasso : public MovingObject {
 ...
 Lasso(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta) : MovingObject(speed, angle_deg, argax, argay, argpaused, rtheta) {
  ...
  void nextStep(double t);
  void check_for_coin(Coin *coin,int coin_no);
...
void Coin::initCoin() {
  coin_start_x = (PLAY_X_START+WINDOW_X)/2 + rand()%50;     //NEAR THE CENTRE OF X AXES
  coin_start_y = PLAY_Y_HEIGHT;
  coin_circle.reset(coin_start_x, coin_start_y, COIN_SIZE);
  coin_circle.setColor(COLOR("black"));     //COIN COLOUR
...
void Lasso::initLasso() {
  ...
  lasso_loop.setColor(COLOR("pink"));
  ...
  lasso_line.setColor(COLOR("pink"));
...
void Lasso::yank() {        //PULLS THE LASSO BACK AND RESETS THE COINS COLLECTED/ATTACHED, IF ANY
  bool paused = true, rtheta = true;
...
void Lasso::check_for_coin(Coin *coinPtr, int coin_no = 0) {
  ...
  if( distance <= LASSO_RADIUS && !(coinPtr->isPaused()) )  {
    the_coin[coin_no] = coinPtr;
    the_coin[coin_no]->getAttachedTo(this);
  }
...
main_program {
  ...
  char coinScoreStr[256], nlife[256];
  sprintf(coinScoreStr, "Coins: %d", lasso.getNumCoins());
  sprintf(nlife, "Lives: %d", no_of_lives);
  Text coinScore(PLAY_X_START+50, PLAY_Y_HEIGHT+50, coinScoreStr);
  Text lives(WINDOW_X-150, 30, nlife);
