...
class Coin : public MovingObject {
 ...
 Coin(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta, int id, bool mag=false) : MovingObject(speed, angle_deg, argax, argay, argpaused, rtheta) {
    release_speed = speed;
...
main_program {
  ...
  sprintf(mag_time_str, "Magnet: %d second(s) left.", MAG_TIME - int(lasso.getMagTimer())  );
  Text coinScore(PLAY_X_START+50, PLAY_Y_HEIGHT+50, coinScoreStr);
  Text lives(WINDOW_X-150, 20, nlife), mag_Time(WINDOW_X-150, 40, mag_time_str);
